...
const IS_DEV = process.env.NEXT_PUBLIC_DEVELOPMENT_MODE === 'true';
...
export default function PracticePage() {
  ...
  const handleSubmit = async (answerPayload = userAnswer) => {
    ...
    try {
      ...
      if (correct) {
        setShowPraise(true);
        await delay(1800); // Allow praise animation to finish
    ...
    <div className={styles.container}>
          ...
          <div className={styles.topBarStats}>
            ...
            {IS_DEV && (
              <button
                ...
                title={showDebugJson ? 'Show regular view' : 'View JSON structure'}
                style={{
                  background: '#334155',
                  border: 'none',
                  borderRadius: '8px',
      ...
      <div className={`${styles.layout} ${isJourney ? styles.fullWidthLayout : ''}`}>
        <main className={styles.mainContent}>
          ...
          {/* Checking your answer & Great job feedback now handled by QuestionRenderer SuccessFeedback */}
          {/*
          ...
          */}
          ...
          {isAnswered && isCorrect === false && (feedbackData?.intervention === 'SCAFFOLD' || feedbackData?.scaffold || currentQuestion?.adaptiveConfig?.scaffold) && (
            <div className={`${styles.feedback} ${styles.scaffold} ${styles.incorrectDetailed}`}>
              <div className={styles.scaffoldHeader}>
                ...
                <h2 className={styles.scaffoldTitle}>{feedbackData?.message || (feedbackData?.scaffold || currentQuestion?.adaptiveConfig?.scaffold)?.title || 'Let\'s try a hint!'}</h2>
              </div>

              <div className={styles.scaffoldBody}>
                {(() => {
                  const scaffold = feedbackData?.scaffold || currentQuestion?.adaptiveConfig?.scaffold;
                  if (scaffold?.parts) {
                    return (
                      <div className={styles.scaffoldParts}>
                        <QuestionParts parts={scaffold.parts} />
                      </div>
                    );
                  }
                  return (
      ...
      <SuccessFeedback 
        show={showPraise} 
        onComplete={() => setShowPraise(false)} 
        score={smartScore}
      />
    </div>
  );
