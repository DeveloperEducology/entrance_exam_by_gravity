...
export async function getCroppedImgBlob(
    ...
    // Create a new canvas for the final cropped (and potentially scaled) image
    ...
    if (targetWidth || targetHeight) {
        // We need to scale. Create a temporary canvas for the intermediate crop
        const tempCanvas = document.createElement('canvas')
        tempCanvas.width = pixelCrop.width
        tempCanvas.height = pixelCrop.height
        tempCanvas.getContext('2d').putImageData(data, 0, 0)
        
        // Draw onto final canvas with scaling
        finalCtx.drawImage(tempCanvas, 0, 0, pixelCrop.width, pixelCrop.height, 0, 0, finalWidth, finalHeight)
    } else {
